...
def update_clothing_item(
    ...
    item_update: schemas.ClothingItemUpdate = Depends(),  # Utiliser le modèle correct pour la mise à jour
    db: Session = Depends(
        get_db
    ),  # S'assurer que get_db est importé correctement depuis le module database
    current_user: models.User = Depends(get_current_user),
):
    db_item = crud.get_clothing_item_by_id(db, item_id=item_id)
    if db_item is None:
    ...
    updated_item = crud.update_clothing_item(
        db, item_id, item_update.dict(), current_user.id
    )
